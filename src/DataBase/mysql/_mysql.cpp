...
/// @brief 此程序是开发框架的C/C++操作mysql数据库的定义文件
/// @version 0.1
/// @date 2023-01-23
///
...
int connection::connecttodb(const char* connstr, const char* charset, unsigned int autocommitopt) {
    ...
    if (mysql_real_connect(
            m_conn, m_env.ip, m_env.user, m_env.pass, m_env.dbname, m_env.port, NULL, 0) == NULL) {
...
sqlstatement::sqlstatement() { initial(); }
...
sqlstatement::~sqlstatement() { disconnect(); }
...
int sqlstatement::bindin(unsigned int position, char* value, unsigned int len) {
    ...
    params_in[position - 1].is_null = (my_bool*)&params_in_is_null[position - 1];
...
int sqlstatement::bindoutlob(unsigned int position,
                             void* buffer,
...
int sqlstatement::execute() {
    ...
    if (mysql_stmt_execute(m_handle) != 0) {
        ...
        if (m_cda.rc == 1243)
            memcpy(&m_cda, &m_cda1, sizeof(struct CDA_DEF));
...
bool buftofile(const char* filename, char* buffer, unsigned long size) {
    ...
    if (rename(filenametmp, filename) != 0)
        return false;
