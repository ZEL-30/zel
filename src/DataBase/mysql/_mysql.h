...
/// @brief 把文件filename加载到buffer中，必须确保buffer足够大
/// @param filename 文件名
/// @param buffer
/// @return unsigned long 成功返回文件的大小，文件不存在或为空返回0
unsigned long filetobuf(const char* filename, char* buffer);

/// @brief 把buffer中的内容写入文件filename
/// @param filename 文件名
/// @param buffer
/// @param size buffer中有效内容的大小
/// @return true 成功
/// @return false 失败
...
/// @brief 每次调用MySQL接口函数返回的结果
struct CDA_DEF {
    int rc; // 返回值：0-成功；其它是失败，存放了MySQL的错误代码。
    unsigned long
        rpc; // 如果是insert、update和delete，存放影响记录的行数，如果是select，存放结果集的行数。
...
class connection {
  ...
  public:
    ...
    /// @brief 登录数据库
    /// @param connstr 数据库的登录参数，格式："ip,username,password,dbname,port",
    /// 例如："172.16.0.15,qxidc,qxidcpwd,qxidcdb,3306"。
    /// @param charset
    /// 数据库的字符集，如"utf8"、"gbk"，必须与数据库保持一致，否则会出现中文乱码的情况
    /// @param autocommitopt 是否启用自动提交，0-不启用，1-启用，缺省是不启用
    /// @return int 0-成功，其它失败，失败的代码在m_cda.rc中，失败的描述在m_cda.message中
    int connecttodb(const char* connstr, const char* charset, unsigned int autocommitopt = 0);

    /// @brief 提交事务
    /// @return int 0-成功，其它失败，程序中一般不必关心返回值
    int commit();
    ...
    /// @brief 断开与数据库的连接
    /// @return int 0-成功，其它失败，程序中一般不必关心返回值
    /// @warning 断开与数据库的连接时，全部未提交的事务自动回滚
    int disconnect();

    /// @brief 执行SQL语句
    ///        如果SQL语句不需要绑定输入和输出变量（无绑定变量、非查询语句），可以直接用此方法执行
    /// @param fmt 这是一个可变参数，用法与printf函数相同
    /// @return int 0-成功，其它失败，失败的代码在m_cda.rc中，失败的描述在m_cda.message中,
    /// 如果成功的执行了非查询语句，在m_cda.rpc中保存了本次执行SQL影响记录的行数。
    /// @warning 程序中必须检查execute方法的返回值
    /// @note
  ...
  private:
    /// @brief 从connstr中解析ip,username,password,dbname,port
    /// @param connstr 连接数据库的参数字符串
    void setdbopt(const char* connstr);

    /// @brief 设置字符集，要与数据库的一致，否则中文会出现乱码
    /// @param charset
...
/// @brief 执行SQL语句前绑定输入或输出变量个数的最大值，256是很大的了，可以根据实际情况调整
...
class sqlstatement {
  ...
  public:
    ...
    /// @return int
    /// 0-成功，其它失败，只要conn参数是有效的，并且数据库的游标资源足够，connect方法不会返回失败
    /// @warning 每个sqlstatement只需要绑定一次，在绑定新的connection前，必须先调用disconnect方法,
    ...
    /// @param ...
    /// @return int 0-成功，其它失败，程序中一般不必关心返回值
    /// @warning 如果SQL语句没有改变，只需要prepare一次就可以了
    ...
    /// @return int 0-成功，其它失败，程序中一般不必关心返回值
    /// @warning
    /// 1）如果SQL语句没有改变，只需要bindin一次就可以了，2）绑定输入变量的总数不能超过MAXPARAMS个
    ...
    /// @return int 0-成功，其它失败，程序中一般不必关心返回值
    /// @warning
    /// 1）如果SQL语句没有改变，只需要bindout一次就可以了，2）绑定输出变量的总数不能超过MAXPARAMS个
    ...
    int
    bindoutlob(unsigned int position, void* buffer, unsigned long buffersize, unsigned long* size);
    ...
    /// @return int
    /// 0-成功，1403-结果集已无记录，其它-失败，失败的代码在m_cda.rc中，失败的描述在m_cda.message中
