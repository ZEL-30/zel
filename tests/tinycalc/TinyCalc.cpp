...
class interpreter {
    ...
    vector<SYMBOL> registered_symbols;

    vector<map<string, SYMBOL>> vars;

    map<string, vector<string>> usr_func;

    vector<pair<int, string>> err_msg;

    bool test_mode, err_strict;

    int max_solve_loop, max_integrate_loop;
  ...
  public:
    ...
    void make_symb_and_push(string sym, int typ, int pri, int pos = 0, double val = 0) {
        registered_symbols.push_back(make_symb(sym, typ, pri, pos, val));
    }

    void init_buildin_symbols() {
    ...
    int next_token(int& x, string& expr) {
        int res = expr.size(), cnt;
        string temp;
    ...
    string get_string(string& expr, int& addr) {
        string res;
        if (expr[addr] == '\"')
            addr++;
    ...
    double get_number(string& expr, int& addr) {
        double res, base = 0.1;
        long double tres = 0;
        ...
        for (; addr < expr.size(); addr++) {
            ...
            if (abs(tres) >= 1.7976931348623157E+308)
                err_msg.push_back(
                    make_pair(addr, "Warning : Value too large. Mistakes may appear."));
        }
        res = (double)tres;
        return res;
    }

    void print_list(vector<SYMBOL> t) {
        if (!t.size()) {
            cout << "NULL";
            return;
        }
    ...
    void lexer(vector<SYMBOL>& tokens, string& expr) {
        double res;
        int nxt_token, nxt_symb;
        string temp;
        ...
        if (!tokens.empty() && (tokens[0].sym == "-" || tokens[0].sym == "+"))
        ...
        for (int i = 1; i < tokens.size(); i++) {
            ...
            if ((!tokens[i - 1].typ && (tokens[i].typ >= 3 && tokens[i].typ <= 5)) ||
                (!tokens[i].typ && (tokens[i - 1].typ >= 3 && tokens[i - 1].typ <= 5)) ||
    ...
    void executer(vector<SYMBOL>& tokens, vector<SYMBOL>& res) {
        ...
        for (int i = 0; i < tokens.size(); i++) {
            ...
            if (tokens[i].typ == 5 || tokens[i].typ == 4 || tokens[i].typ == 9) {
                ...
                if (!flag) {
                    ...
                    // err_msg.push_back(make_pair(tokens[i].pos,"Warning : Undefined var
                    // \"" + tokens[i].sym +"\". Initial value is 0."));
                }
                val_stack.push(tmp);
                continue;
            } // custom values

            if (tokens[i].typ == 1 || tokens[i].typ == 2 || tokens[i].typ == 3 ||
                tokens[i].typ == 8) {

                if (tokens[i].sym == "While" || tokens[i].sym == "Do" || tokens[i].sym == "Solve" ||
                    tokens[i].sym == "Integrate" || tokens[i].sym == "Diff" ||
                    tokens[i].sym == "Sum" || tokens[i].sym == "Quadrature") {
                    loop_pos.push(i);
                ...
                if (tokens[i].typ != 8 &&
                    (opr_stack.empty() || opr_stack.top().pri > tokens[i].pri)) {
                    opr_stack.push(tokens[i]);
                    continue;
                } else {
                    while (!opr_stack.empty() &&
                           (tokens[i].sym == ")" ? (opr_stack.top().sym != "(")
                                ...
                                if ((a[ia].typ == 0) && (b[ib].typ == 0) ||
                                    ...
                                    if (temp.sym == "+") {
                                        ...
                                        if (b[ib].typ == 0 || b[ib].typ == 4 || b[ib].typ == 5) {
                                            ...
                                            err_msg.push_back(make_pair(
                                                temp.pos, "Error : Mismatch argument type."));
                                    } else
                                        err_msg.push_back(make_pair(temp.pos,
                                                                    "Error : \"" + temp.sym +
                                                                        "\" is an unknow symbol."));
                                    if (flag)
                                        c.push_back(a[ia]);
                                    ...
                                    if (temp.sym == "+")
                                        ...
                                        err_msg.push_back(make_pair(
                                            temp.pos, "Error : " + temp.sym + " : Unknow symbol."));
                                    if (flag)
                                    ...
                                    err_msg.push_back(
                                        make_pair(temp.pos, "Error : Mismatch object types."));
                                }
                            }
                            val_stack.push(c);
                        }
                        ...
                        if (temp.typ == 3) {
                            if (val_stack.empty()) {
                                err_msg.push_back(
                                    make_pair(temp.pos, "Error : Missing function argument."));
                                continue;
                            }
                            a = val_stack.top();
                            val_stack.pop();
                            ...
                            if (temp.sym == "Sort") {
                                ...
                                if (a.size() != 2) {
                                    err_msg.push_back(make_pair(
                                        temp.pos, "Error : Incorrect argument quantity."));
                                    a.push_back(make_symb("null", 7, -1, -1, 0));
                                    flag = 1;
                                }
                                if (flag || !(a[0].typ == 5 || a[0].typ == 9) ||
                                    (a[1].typ != 4 && a[1].typ != 0 && a[1].typ != 5 &&
                                     a[1].typ != 6)) {
                                    err_msg.push_back(
                                        make_pair(temp.pos, "Error : Incorrect argument type."));
                                    a.push_back(make_symb("null", 7, -1, -1, 0));
                                    flag = 1;
                                }
                                if (!flag) {
                                ...
                                if (flag || loop_pos.empty()) {
                                    err_msg.push_back(
                                        make_pair(temp.pos, "Error : Wrong loop structure."));
                                    flag = 1;
                                }
                                if (!flag) {
                                    if (a[0].val) {
                                ...
                                for (int j = 0; j < a.size(); j++)
                                    if (a[j].typ == 5 || a[j].typ == 9) {
                                        ...
                                        err_msg.push_back(
                                            make_pair(temp.pos, "Error : Invalid object type"));
                                        flag = 0;
                                    }
                                a.clear();
                                ...
                                a.clear();
                                a.push_back(
                                    make_symb("$number", 0, -1, -1, vars[vars.size() - 1].size()));
                                ...
                                if (!flag) {
                                    if (a[0].val >= a[1].dat.size()) {
                                        err_msg.push_back(
                                            make_pair(temp.pos, "Error : Out of range."));
                                        a.push_back(make_symb("null", 7, -1, -1, 0));
                                    } else {
                                        sa.clear();
                                ...
                                if (a[0].typ != 5 || a[1].typ == 6 || a[2].typ == 6 ||
                                    a[3].typ == 6 || a[1].typ == 9 || a[2].typ == 9 ||
                                    a[3].typ == 9 || !is_integer(a[1].val) ||
                                    !is_integer(a[2].val) || (a[1].val > a[2].val) || flag) {
                                    err_msg.push_back(
                                        make_pair(temp.pos, "Error : Incorrect argument type."));
                                    a.push_back(make_symb("null", 7, -1, -1, 0));
                                    flag = 1;
                                ...
                                for (int j = 0; j < a.size(); j++) {
                                    if (a[j].typ != 9 && a[j].typ != 5)
                                        err_msg.push_back(
                                            make_pair(temp.pos, "Error : Mismatch argument type."));
                                    else {
                                        cin >> sa;
                                        ia = 0;
                                        if (is_number(sa[0])) {
                                ...
                                for (int j = 0; j < a.size(); j++) {
                                    if (!a[j].typ || a[j].typ == 4 || a[j].typ == 5) {
                                        ...
                                        if (temp.sym == "Sin")
                                            ...
                                            err_msg.push_back(make_pair(
                                                temp.pos, temp.sym + " : Unknow symbol."));
                                        a[j].val = ta;
                                        a[j].typ = 0;
                                        a[j].sym = "$number";
    ...
    void test_mode_on() { test_mode = 1; }

    void test_mode_off() { test_mode = 0; }

    void err_strict_on() { err_strict = 1; }

    void err_strict_off() { err_strict = 0; }
...
int main() {
    ...
    bool flag = 0;
    // intp.test_mode_on();
    // MessageBox(NULL,"Kernel started.","TinyCalc
    // Kernel",MB_ICONINFORMATION|MB_OK);
